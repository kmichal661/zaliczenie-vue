...
const WATCHLISTS_QUERY = gql`
  query Watchlists {
    watchlists {
      watchlistId
      watched
      movie { movieId title posterUrl releaseYear averageRating }
      user { userId }
    }
  }
`;
...
const { mutate: deleteWatchlist, loading: deleting } = useMutation(DELETE_WATCHLIST);
const { mutate: updateWatchlist, loading: updating } = useMutation(UPDATE_WATCHLIST);
...
const toggleWatched = async (w: any) => {
  try {
    await updateWatchlist({ id: w.watchlistId, input: { watched: !w.watched } });
    await refetch();
  } catch (e) { console.error(e); }
};
</script>
...
<template>
  <div class="p-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <Card v-for="w in myWatchlists" :key="w.watchlistId" class="cursor-pointer">
        <CardContent class="p-0">
          <img :src="w.movie.posterUrl" class="w-full h-56 object-cover rounded-t-lg" />
          <div class="p-3 space-y-1">
            <div class="flex items-center justify-between">
              <div class="font-semibold text-sm">{{ w.movie.title }}</div>
              <div class="text-xs">⭐ {{ w.movie.averageRating ?? 'N/A' }}</div>
            </div>
            <div class="text-xs text-muted-foreground">{{ w.movie.releaseYear }}</div>

            <div class="mt-2 flex items-center gap-2">
              <Button @click="() => router.push(`/movie/${w.movie.movieId}`)">Details</Button>
              <Button variant="ghost" @click="() => toggleWatched(w)">{{ w.watched ? 'Watched' : 'Mark watched' }}</Button>
              <Button variant="destructive" @click="() => removeFromWatchlist(w.watchlistId)">Remove</Button>
            </div>
          </div>
        </CardContent>
      </Card>
    ...
    <div v-if="!myWatchlists.length" class="text-sm text-muted-foreground mt-4">Your watchlist is empty.</div>
  </div>
</template>